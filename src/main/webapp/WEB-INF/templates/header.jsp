		...
		<div class="collapse navbar-collapse" id="navbarResponsive">
			<ul class="navbar-nav ml-auto">
				...
				<sec:authorize access="isAuthenticated()">
					<li class="nav-item">[<sec:authentication property="principal.name" />님 안녕하세요]
					</li>
				</sec:authorize>
				...
				<sec:authorize access="isAuthenticated()">
					<li class="nav-item"><c:url var="logoutUrl" value="/logout" />
						<form style="display: contents;" action="${logoutUrl}" method="post">
							<input type="submit" class="customButton" value="Logout" /> <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />
						</form></li>
				</sec:authorize>
			</ul>
		</div>
